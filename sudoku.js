...
var alertGameChangesContinueButtonElement = document.querySelector("#continue-button-game-changes");
var alertGameChangesNextGameButtonElement = document.querySelector("#new-game-button");
...
var alertGameChanges = {
  ...
  pressed: false,
  buttons: [alertGameChangesContinueButtonElement, alertGameChangesNextGameButtonElement],
  buttonPressed: "easy"
}
var hint = {
  element: hintElement,
...
function insertingDivs() {
  for (var i = 0; i < columns; i++) {
    for (var j = 0; j < columns; j++) {
      gameContainer.insertAdjacentHTML("beforeend", '<div class="block" id="' + i + '-' + j + '"></div>');
    }
  }
...
for (var i = 0; i < 9; i++) {
  numberButons[i].addEventListener("mouseup", function (e) {
    ...
    if (blocks[iSelected][jSelected].generatedNumber != true && !blocks[iSelected][jSelected].hintedNumber) {
      switch (e.target.id) {
        case "1":
          selectedNumber = 1;
          break;
...
function insertingNumber(selectedNumber) {
  if (!notes.toggled) {
    ...
    if (!blocks[iSelected][jSelected].notes[selectedNumber - 1].toggled) {
      blocks[iSelected][jSelected].notes[selectedNumber - 1].element.innerHTML = selectedNumber;
      blocks[iSelected][jSelected].notes[selectedNumber - 1].toggled = true;
    } else {
      blocks[iSelected][jSelected].notes[selectedNumber - 1].number.innerHTML = "";
  ...
  for (var i = 0; i < 9; i++) {
    for (var j = 0; j < 9; j++) {
      if (currentSudokuGame.charAt(letterTracker) != ".") {
        blocks[i][j].element.innerText = currentSudokuGame.charAt(letterTracker);
        ...
        blocks[i][j].element.innerHTML = '<div class="number"></div><div class="notes-container" id="' + i + '-' + j + '"><div id="1"></div><div id="2"></div><div id="3"></div><div id="4"></div><div id="5"></div><div id="6"></div><div id="7"></div><div id="8"></div><div id="9"></div></div>'
      }
      letterTracker++;
    }
  }
    ...
    for (var j = 0; j < 9; j++) {
      for (var notesTracker = 0; notesTracker < 9; notesTracker++) {
        if (!blocks[i][j].generatedNumber) {
          ...
          blocks[i][j].notes[notesTracker].element = allNoteNumbers[numberTracker];
...
function selectingNumbers(iSelect, jSelect) {
  iSelected = iSelect;
  jSelected = jSelect;
  if (!blocks[iSelect][jSelect].generatedNumber && !blocks[iSelect][jSelect].selectedAsWrong) {
    blocks[iSelect][jSelect].element.classList.add("selected-number-right");
  }
  for (var i = 0; i < columns; i++) {
    if (blocks[iSelect][i].generatedNumber && blocks[iSelect][i].selectedAsWrong == false) {
      blocks[iSelect][i].element.classList.add("selected-generated-number-field");
    } else if (blocks[iSelect][i].selectedAsWrong == false) {
      blocks[iSelect][i].element.classList.add("selected-field");
    }
    if (blocks[i][jSelect].generatedNumber && blocks[i][jSelect].selectedAsWrong == false) {
      blocks[i][jSelect].element.classList.add("selected-generated-number-field");
...
addEventListener("keydown", function (e) {
  switch (e.keyCode) {
    ...
    case 40:
      ...
      break;
    case 48:
      if (iSelected != undefined && jSelected != undefined && !blocks[iSelected][jSelected].generatedNumber && !blocks[iSelected][jSelected].hintedNumber) {
...
function checkSelectedNumber(iSelected, jSelected, numberPressed) {
  if (blocks[iSelected][jSelected].value != undefined) {
    for (var i = 0; i < columns; i++) {
      if (blocks[iSelected][i].value == numberPressed && blocks[iSelected][i].element != blocks[iSelected][jSelected].element) {
        makingBlockRed(iSelected, i, true);
        makingBlockRed(iSelected, jSelected, false);
      }
      if (blocks[i][jSelected].value == numberPressed && blocks[i][jSelected].element != blocks[iSelected][jSelected].element) {
    ...
    for (i = iStart; i < iBorder; i++) {
      for (j = jStart; j < jBorder; j++) {
        if (blocks[i][j].value == numberPressed && blocks[i][j].element != blocks[iSelected][jSelected].element) {
...
function makingBlockRed(i, j, sideButton) {
  blocks[i][j].element.classList.remove("selected-field");
  blocks[i][j].element.classList.remove("selected-generated-number-field");
  if (sideButton && (blocks[i][j].generatedNumber || blocks[i][j].hintedNumber)) {
...
function removeRedBlocks(iSelected, jSelected) {
  for (var i = 0; i < columns; i++) {
    blocks[iSelected][i].element.classList.remove("selected-button-wrong-side-blocks");
    ...
    blocks[i][jSelected].element.classList.remove("selected-button-wrong-side-blocks");
  ...
  for (i = iStart; i < iBorder; i++) {
    for (j = jStart; j < jBorder; j++) {
      if (i == iSelected && j == jSelected) {
        ...
        blocks[i][j].element.classList.remove("selected-button-wrong-side-blocks");
      }
  ...
  if (time.gameStarted) {
    if (!alertTime.pressed) {
      ...
      if (time.timerSeconds < 10) {
        ...
        time.display.innerHTML = time.timerMinutes + ":" + time.timerSeconds;
      }
    }
  }
}, 1000);
...
function revealingOneNumber() {
  ...
  if (blocks[iRandom][jRandom].generatedNumber || blocks[iRandom][jRandom].hintedNumber || blocks[iRandom][jRandom].value != undefined) {
    if (fieldsFilled != 81) {
      revealingOneNumber()
    } else {
      winningGame();
    ...
    blocks[iRandom][jRandom].number.innerHTML = blocks[iRandom][jRandom].solution;
...
function winningGame() {
  if (fieldsFilled == 81) {
    ...
    for (var i = 0; i < 9; i++) {
      for (var j = 0; j < 9; j++) {
        if (blocks[i][j].value != currentSudokuSolution.charAt(letterTracker)) {
          return;
        }
        letterTracker++;
      }
...
function gameWon() {
  ...
  if (time.timerSeconds < 10) {
    gameWonAlerter.time.innerHTML = "time:  " + time.timerMinutes + ":0" + time.timerSeconds;
  } else {
    gameWonAlerter.time.innerHTML = "time:  " + time.timerMinutes + ":" + time.timerSeconds;
  }
}
